...
from smart_hub.stock import (
    configure as configure_stock_routes,
    router as stock_router,
)
import smart_hub.coin as coin_module
from smart_hub.coin import (
    configure as configure_coin_routes,
...
# ── 자동매매 주문 조회 엔드포인트 (v2) ──────────────────────────
@stock_router.get("/api/autotrade/orders")
...
@stock_router.get("/api/autotrade/debug")
...
@stock_router.get("/api/autotrade/summary")
...
@stock_router.get("/stock-recommend")
...
@stock_router.get("/api/sector-heatmap")
...
@stock_router.get("/api/investor-flow")
...
@stock_router.get("/api/stock-signals")
...
@stock_router.get("/api/stock-alerts")
...
@stock_router.get("/api/autotrade/config")
...
@stock_router.post("/api/autotrade/config")
...
@stock_router.post("/api/autotrade/kill")
...
@stock_router.get("/api/stock-signals/performance")
...
@stock_router.get("/api/stock-signals/{signal_id}/track")
...
async def _start_background_tasks():
    await coin_module.start_coin_autotrade_monitor()


configure_stock_routes(SimpleNamespace(
    verify_firebase_token=verify_firebase_token,
    ensure_admin=_ensure_admin,
    fetch_quote=_fetch_quote,
    is_market_hours=_is_market_hours,
    get_firestore=_get_firestore,
    paper_today_stats=_paper_today_stats,
    paper_stock_price=_paper_stock_price,
    paper_apply_position_txn=_paper_apply_position_txn,
    lookup_symbol_name=_lookup_symbol_name,
    kis_account_cache=_kis_account_cache,
    firestore=_firestore,
    doc_to_dict=_doc_to_dict,
    kis_account_configured=_kis_account_configured,
    fetch_kis_account_snapshot=_fetch_kis_account_snapshot,
    stock_search_cache=_stock_search_cache,
    stock_search_cache_ttl=STOCK_SEARCH_CACHE_TTL,
    stock_search_cache_max=STOCK_SEARCH_CACHE_MAX,
    fetch_naver_stock_search=_fetch_naver_stock_search,
    remember_symbol_name=_remember_symbol_name,
))
configure_coin_routes(SimpleNamespace(
    verify_firebase_token=verify_firebase_token,
    ensure_admin=_ensure_admin,
    get_firestore=_get_firestore,
    ...
    firestore=_firestore,
    doc_to_dict=_doc_to_dict,
    compute_perf_stats=_compute_perf_stats,
