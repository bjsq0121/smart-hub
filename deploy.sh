...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --cpu 2 \
  --timeout 300 \
  --set-env-vars "NAVER_CLIENT_ID=${NAVER_CLIENT_ID},NAVER_CLIENT_SECRET=${NAVER_CLIENT_SECRET},MOLIT_API_KEY=${MOLIT_API_KEY},GCP_PROJECT=${PROJECT_ID},TELEGRAM_TOKEN=${TELEGRAM_TOKEN},TELEGRAM_CHAT_ID=${TELEGRAM_CHAT_ID},WEBHOOK_SECRET=${WEBHOOK_SECRET},KIS_APP_KEY=${KIS_APP_KEY},KIS_APP_SECRET=${KIS_APP_SECRET},KIS_ACCOUNT_NO=${KIS_ACCOUNT_NO},KIS_ACCOUNT_PROD=${KIS_ACCOUNT_PROD:-01},UPBIT_ACCESS_KEY=${UPBIT_ACCESS_KEY},UPBIT_SECRET_KEY=${UPBIT_SECRET_KEY}" \
  --min-instances=1 \
  --project "${PROJECT_ID}"
